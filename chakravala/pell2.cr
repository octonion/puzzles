...
def check(i,p,factors)
  ...
  return i, BigInt.new(e)
end
  
def prime_factors_faster(i)
  factors = [] of {BigInt,BigInt}
  q, e = check(i,2,factors)
  if (e>0)
    factors << {BigInt.new(2),e}
  end
  q, e = check(q,3,factors)
  if (e>0)
    factors << {BigInt.new(3),e}
  end
  p = 5
  while p * p <= q
    q, e = check(q,p,factors)
    if (e>0)
      factors << {BigInt.new(p),e}
    end
    p += 2
    q, e = check(q,p,factors)
    if (e>0)
  ...
  factors << {BigInt.new(q),BigInt.new(1)} if q > 1
  factors
end
...
def squarefree_part(x)
  sf = 1
  factors = prime_factors_faster(x)
  factors.each do |f|
    if (f[1] % 2==1)
      sf = sf*f[0]
    end
...
#arg = BigInt.new(609)*BigInt.new(7766)*BigInt.new(2**2)*BigInt.new(4657**2)
arg = BigInt.new(ARGV[0])
...
if (s>1)
  ...
  rs = BigInt.new(Math.sqrt(s).floor)
  rs_f = prime_factors_faster(rs)
